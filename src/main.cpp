...
#if defined(NDEBUG)
# error "Raiucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb1b700ed3d55141d1d2752737128c95a508506fe8640c32972c1ec2985c78762");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Raiucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Raiucoin Signed Message:\n";
...
{
    // Raiucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525600); // Raiucoin: 525600 blocks in 1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Raiucoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Raiucoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Raiucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Raiucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6edbe07aa066daaf1b9bd4a5cf110a895ec8f85af86771ccd35f6bb8538dfdc0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "In order to get Rocket Launcher in RE2, you should play Hardcore with Leon and clear S+";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1549000663;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085096536;

        if (fTestNet)
        {
            block.nTime    = 1549000641;
            block.nNonce   = 387231286;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9b81b96cb84491b961792e9d4d8da3010f0700e0c895f3850f70ba67574a3183"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Raiucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RaiucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RaiucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RaiucoinMiner(CWallet *pwallet)
{
    printf("RaiucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("raiucoin-miner");
    ...
    try { loop {
        ...
        printf("Running RaiucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RaiucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RaiucoinMiner, pwallet));
}
