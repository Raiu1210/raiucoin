...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Raiucoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f, 0x31f1ef0a, 0x3c799462
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Raiucoin is probably already running."), addrBind.ToString().c_str());
        else
